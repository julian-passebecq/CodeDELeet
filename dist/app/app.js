...
function persist(immediate = false) { clearTimeout(saveTimer); const write = () => { store.settings.workstation = presentation; if (storageBlocked) {
    ...
    toast('Browser storage is unavailable or full. Export a backup from Settings before leaving.', true);
} }; if (immediate)
    write();
else
...
function git() { const s = draft().labState; if (s?.kind === 'git' && s.value?.commits) {
    try {
        ...
        toast('Saved virtual Git state is invalid. Reset this lab or export a backup.', true);
    }
...
async function run() {
    ...
    try {
        ...
        if (q.executionMode === 'execute') {
            if (!store.settings.runtimeConsent) {
                if (!confirm('Load a pinned browser runtime from jsDelivr? This can download substantial WebAssembly assets. Only run code you trust; do not put secrets in a browser code lab.'))
                    throw Error('Runtime download was not authorized.');
                store.settings.runtimeConsent = true;
                persist(true);
            }
            const progress = (m) => { if (token === runEpoch) {
                $('#engine-status').textContent = m;
            ...
            r = { engine: 'Deterministic Git simulation', mode: 'simulate', columns: [], rows: [], elapsedMs: 0, checks, notice: 'These checks examine the actual virtual repository state, not your command text. No GitHub access.' };
        }
        else if (q.renderer === 'terminal') {
            const state = terminal(), last = terminalResult;
            const checks = last ? terminalGoal(q, last.value, last.output) : [];
            r = { engine: 'Virtual ' + state.shell, mode: 'simulate', columns: [], rows: [], elapsedMs: 0, checks, notice: last ? 'Checked the last command output against the exercise goal. No host commands ran.' : 'Run a shell command first; no output has been checked.' };
        }
        else if (q.renderer === 'dag-editor') {
            runGrid = simulateDAG(graph());
            runStatuses = Object.fromEntries(runGrid.map(r => [r.id, r.status]));
            const checks = q.fixture?.targetTask ? [{ label: 'Join completes after an intentional skip', passed: runGrid.find(r => r.id === q.fixture?.targetTask)?.status === 'success', detail: 'Choose a trigger rule compatible with the fixture skip.' }] : graphChecks(graph(), q.template ?? 'pipeline');
            r = { engine: 'DAG teaching simulator', mode: 'simulate', columns: ['id', 'status', 'start', 'end', 'attempts', 'message'], rows: runGrid.map(x => [x.id, x.status, x.start, x.end, x.attempts, x.message]), elapsedMs: 0, checks, notice: 'All times are deterministic fixture values. Airflow/ADF are not running. Structural repair and run success are separate observations.' };
            renderLab();
        }
        else if (q.renderer === 'semantic-model') {
            if (q.engine === 'dax-subset') {
                const ctx = { country: draft().country ?? 'All', category: draft().category ?? 'All' }, actual = evaluateDax(code(), fixtures, graph(), ctx);
                const expected = evaluateDax(q.solution, fixtures, graph(), ctx);
                r = { engine: 'Bounded DAX teaching interpreter', mode: 'simulate', columns: ['Measure', 'Value', 'Visible rows'], rows: [[actual.name, actual.value === null ? 'BLANK' : actual.value, actual.rowCount]], elapsedMs: 0, checks: [{ label: 'Matches reference measure in current filter context', passed: actual.value === expected.value, detail: 'A current-context check is not a proof for every possible model or filter.' }], notice: 'Only the displayed DAX subset and fixture model are supported. No Power BI engine is connected.' };
                const el = document.querySelector('#kpi-value');
                if (el)
                    el.textContent = formatNumber(actual.value);
            }
            else
                r = { engine: 'Star-schema teaching checks', mode: 'simulate', columns: [], rows: [], elapsedMs: 0, checks: modelChecks(graph(), fixtures, draft().grain ?? ''), notice: 'Checks cover supplied keys, explicit active relationships and selected grain, not a production semantic model.' };
        }
        else if (q.renderer === 'config-editor')
            r = { engine: 'Configuration text analyzer', mode: 'analyze', columns: [], rows: [], elapsedMs: 0, checks: configurationChecks(q, code()), notice: 'Partial text checks, not YAML/HCL validation or vendor execution. The evidence fixture remains unchanged.' };
        else if (q.renderer === 'performance-investigation') {
            const answer = q.fixture?.diagnoses?.find((x) => x.id === draft().answer);
            r = { engine: 'Evidence-based Spark review', mode: 'review', columns: [], rows: [], elapsedMs: 0, checks: answer ? [{ label: 'Diagnosis fits supplied evidence', passed: answer.correct === true, detail: 'Explain the causal link and what you would measure next. This does not benchmark your proposed fix.' }] : [], notice: answer ? 'Recorded the selected diagnosis and your reasoning.' : 'Choose a diagnosis before checking it.' };
        }
        else if (q.renderer === 'multi-choice-reasoning')
            r = { engine: 'Reasoning check', mode: 'review', columns: [], rows: [], elapsedMs: 0, checks: draft().answer ? [{ label: 'Selected explanation', passed: draft().answer === q.fixture?.answer, detail: 'Review the explanation after making your own case.' }] : [], notice: 'No runtime execution. Reasoning is saved separately from the selected choice.' };
        else if (q.renderer === 'architecture-editor')
        ...
        if (token === runEpoch) {
            busy = false;
            $('#engine-status').textContent = result?.error ? 'Runtime unavailable / attempt error' : result?.engine ?? 'Ready';
            renderHeader();
            renderDrawer();
            renderLibrary();
            applyLayout();
...
async function renderMermaid() { const source = $('#mermaid-source').value; if (source.length > 15000) {
    ...
    return;
} patch({ mermaid: source }, true); const host = $('#mermaid-output'), qid = current.id; host.textContent = 'Loading optional Mermaid renderer...'; try {
    const url = `https://cdn.jsdelivr.net/npm/mermaid@${RUNTIME_VERSIONS.mermaid}/dist/mermaid.esm.min.mjs`;
    const m = (await import(/* @vite-ignore */ url)).default;
    m.initialize({ startOnLoad: false, securityLevel: 'strict', theme: 'neutral', suppressErrorRendering: true, maxTextSize: 15000, htmlLabels: false, flowchart: { htmlLabels: false } });
    const { svg } = await m.render(newId('diagram'), source);
    if (current.id === qid && host.isConnected) {
        const parsed = new DOMParser().parseFromString(svg, 'image/svg+xml');
        if (parsed.querySelector('script,foreignObject,image'))
            throw Error('This preview disallows scripts, embedded HTML and external images. Use plain Mermaid labels.');
        host.innerHTML = svg;
    }
}
catch (error) {
    if (host.isConnected)
        host.innerHTML = notice('Mermaid preview unavailable', error.message + ' Your source is preserved. The offline canvas and .mmd export still work.', 'error');
} }
function command(which) {
    if (which === 'git') {
        const input = $('#git-command'), text = input.value.trim();
        if (!text)
            return;
        const r = runGit(git(), text);
        patch({ labState: { kind: 'git', value: r.state }, status: 'in-progress' }, true);
        renderLab();
        if (!r.ok)
            toast(r.output, true);
        $('#git-command')?.focus();
        ...
        const input = $('#terminal-command'), text = input.value.trim();
        if (!text)
            return;
        const r = runTerminal(terminal(), text);
        patch({ labState: { kind: 'terminal', value: r.state }, status: 'in-progress' }, true);
        terminalResult = r.ok ? { value: r.value, output: r.output } : null;
        renderLab();
        $('#terminal-command')?.focus();
    }
    const console = document.querySelector('.terminal-console');
    if (console)
...
function bindControls() {
    ...
    app.onclick = (event) => {
        ...
        if (button.dataset.leftTab) {
            leftTab = button.dataset.leftTab;
            renderQuestion();
            return;
        }
        ...
        try {
            switch (action) {
                ...
                case 'export-backup':
                    ...
                    break;
                case 'bookmark':
                    patch({ bookmark: !draft().bookmark }, true);
                    renderHeader();
                    renderLibrary();
                    break;
                case 'timer':
                ...
                case 'add-node': {
                    ...
                    if (g.nodes.length >= 40)
                        throw Error('Teaching limit: 40 nodes.');
                    const id = newId('node');
                    g.nodes.push({ id, label: 'New component', role: current.workspace === 'model' ? 'dimension' : 'transform', x: 35, y: 55 + Math.floor(g.nodes.length / 3) * 155, duration: 3, retries: 0 });
                    selectedNode = id;
                    setGraph(g);
                    break;
                }
                case 'delete-node': {
                    const g = graph();
                    g.nodes = g.nodes.filter(n => n.id !== selectedNode);
                    g.edges = g.edges.filter(x => x.from !== selectedNode && x.to !== selectedNode);
                    selectedNode = '';
                    setGraph(g);
                    break;
                }
                case 'connect-nodes': {
                    const g = graph(), from = $('#edge-from').value, to = $('#edge-to').value;
                    if (from === to)
                        throw Error('Choose two different nodes.');
                    if (g.edges.some(x => x.from === from && x.to === to))
                        throw Error('That dependency already exists.');
                    g.edges.push({ id: newId('edge'), from, to, active: true, label: current.renderer === 'semantic-model' ? '1 : *' : undefined });
                    setGraph(g);
                    break;
                }
                case 'apply-script': {
                    const text = $('#diagram-script').value;
                    const g = parseDiagram(text, graph());
                    patch({ diagram: text });
                    setGraph(g);
                    toast('Diagram script applied to the graph.');
                    break;
                }
                case 'apply-graph':
                    ...
                    break;
                case 'export-graph':
                    download(current.id + '-graph.json', JSON.stringify(graph(), null, 2));
                    break;
                case 'export-svg':
                    download(current.id + '-diagram.svg', graphSVG(graph()), 'image/svg+xml');
                    break;
                case 'render-mermaid':
                    void renderMermaid();
                    break;
                case 'export-mermaid':
                    download(current.id + '.mmd', $('#mermaid-source').value, 'text/plain');
                    break;
                case 'export-code':
                    download(current.id + (/python|spark/i.test(current.language) ? '.py' : /sql/i.test(current.language) ? '.sql' : '.txt'), code(), 'text/plain');
                    break;
                case 'embed-deepnote': {
                    const url = safeDeepnoteURL(current.deepnoteEmbedUrl, true);
                    if (url)
                ...
                case 'stage-file': {
                    const s = git();
                    s.files[selectedFile] = $('#virtual-file').value;
                    const r = runGit(s, 'git add "' + selectedFile.replace(/"/g, '') + '"');
                    patch({ labState: { kind: 'git', value: r.state } }, true);
                    toast(r.output, !r.ok);
                    break;
                }
                ...
                case 'check-prediction': {
                    const choices = gitPredictions();
                    const correct = draft().gitPrediction === choices[0].id;
                    $('#prediction-feedback').innerHTML = notice(correct ? 'Prediction matches' : 'Revisit the state transition', correct ? 'Now make the change in the live virtual repository. Prediction does not move any refs.' : 'Track HEAD, the branch ref, the index and the working tree independently.');
                    break;
                }
            }
        }
    ...
    app.onchange = (event) => {
        ...
        try {
            ...
            if (t.id === 'difficulty') {
            ...
            else if (t.id === 'status' || t.id === 'confidence') {
                patch({ [t.id]: t.value }, true);
                renderLibrary();
            }
            else if (t.name === 'git-prediction')
                patch({ gitPrediction: t.value }, true);
            else if (t.name === 'answer' || t.name === 'diagnosis-choice')
                patch({ answer: t.value }, true);
            else if (t.dataset.rubric !== undefined) {
                const values = new Set(draft().rubric ?? []);
                t.checked ? values.add(t.dataset.rubric) : values.delete(t.dataset.rubric);
                patch({ rubric: [...values] }, true);
            }
            else if (t.dataset.edgeActive) {
                const g = graph();
                g.edges.find(x => x.id === t.dataset.edgeActive).active = t.checked;
                setGraph(g);
            }
            else if (t.dataset.edgeCondition) {
                const g = graph();
                g.edges.find(x => x.id === t.dataset.edgeCondition).condition = t.value;
                setGraph(g);
            }
            else if (t.id.startsWith('node-')) {
                const g = graph(), n = g.nodes.find(n => n.id === selectedNode);
                if (!n)
                    return;
                if (t.id === 'node-label')
                    n.label = t.value;
                if (t.id === 'node-role')
                    n.role = t.value;
                if (t.id === 'node-retries')
                    n.retries = Number(t.value);
                if (t.id === 'node-trigger')
                    n.triggerRule = t.value;
                if (t.id === 'node-transient')
                    n.transient = t.checked;
                if (t.id === 'node-skip')
                    n.skip = t.checked;
                setGraph(g);
            }
...
app.addEventListener('keydown', event => { const t = event.target; if (t.dataset.modelEdge && (event.key === 'Enter' || event.key === ' ')) {
    ...
    if (event.key === 'Enter' || event.key === ' ') {
        ...
        event.preventDefault();
        const g = graph(), n = g.nodes.find(n => n.id === t.dataset.node);
        n.x = Math.max(0, n.x + (event.key === 'ArrowLeft' ? -10 : event.key === 'ArrowRight' ? 10 : 0));
        n.y = Math.max(0, n.y + (event.key === 'ArrowUp' ? -10 : event.key === 'ArrowDown' ? 10 : 0));
        selectedNode = n.id;
        setGraph(g);
    }
} });
...
async function boot() {
    try {
        ...
        builtins = await Promise.all(manifest.packs.map(async (path) => validatePack(await (await fetch('./packs/' + path)).json())));
        fixtures = await (await fetch('./packs/fixtures.json')).json();
        mapping = (await (await fetch('./packs/deepnote-mapping.json')).json()).links ?? {};
        try {
