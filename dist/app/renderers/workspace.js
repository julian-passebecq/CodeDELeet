...
export function labTabs(ctx) {
    switch (ctx.current.renderer) {
...
export function renderLabContent(ctx) {
...
export function codePanel(ctx, text) { return `<div class="code-context"><span>${e(ctx.current.language)}</span><small>${e(ctx.current.executionMode === 'execute' ? 'Runtime loads on demand' : 'No remote execution')}</small></div><div id="editor-host" class="editor-host"></div><div class="lab-footnote">${e(text)}</div>`; }
export function semanticKPI(ctx) {
    if (ctx.current.engine !== 'dax-subset')
        return 'Grain first';
    try {
