...
export function companionLinks(ctx) { return validDeepnoteLinks(ctx.current, { ...ctx.mapping, ...ctx.store.settings.deepnoteMap }); }
export function updateRunBadge(ctx) {
    if (!ctx.current)
        return;
...
export function changeMode(ctx, slot) {
...
export function handleShellClick(ctx, button) {
    if (button.dataset.mode) {
        ctx.changeMode(button.dataset.mode);
        return true;
    }
    if (button.dataset.tool) {
        ctx.openTool(button.dataset.tool);
        return true;
    }
    ...
    if (button.dataset.quickFilter) {
        ...
        (ctx.$('#technology')).value = ctx.technology;
        ctx.renderLibrary();
        ctx.renderHeader();
        return true;
    }
    ...
    if (action === 'all-exercises') {
        ...
        ctx.$('#search').value = '';
        ctx.$('#difficulty').value = ctx.difficulty;
        ctx.$('#technology').value = ctx.technology;
        ctx.$('#queue').value = ctx.queue;
        ctx.$('#concept-filter').value = '';
        ctx.$('#priority-filter').value = ctx.priorityFilter;
        ctx.renderLibrary();
        ctx.renderHeader();
    }
...
export function handleShellChange(ctx, t) {
    ...
    if (t.id === 'layout-mode') {
        ...
        m.outputAnchor = t.value;
    else if (t.id === 'output-size')
        setOutputSize(ctx.presentation, ctx.workspace, ctx.shellState, t.value);
