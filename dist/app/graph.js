...
export function graphSVG(g, selected = '', statuses = {}) {
    ...
    const edges = g.edges.map(edge => { const a = g.nodes.find(n => n.id === edge.from), b = g.nodes.find(n => n.id === edge.to); if (!a || !b)
        return ''; const forward = b.x > a.x, x1 = a.x + (forward ? 195 : 0), x2 = b.x + (forward ? 0 : 195), y1 = a.y + 35, y2 = b.y + 35, mid = (x1 + x2) / 2; const path = Math.abs(a.x - b.x) < 30 ? `M${a.x + 97},${a.y + 75} L${b.x + 97},${b.y}` : `M${x1},${y1} C${mid},${y1} ${mid},${y2} ${x2},${y2}`; return `<g><path d="${path}" fill="none" stroke="${edge.active === false ? '#b7bfc0' : '#81948c'}" stroke-width="1.6" ${edge.active === false ? 'stroke-dasharray="5 5"' : ''} marker-end="url(#graph-arrow)"/><text x="${mid}" y="${(y1 + y2) / 2 - 7}" text-anchor="middle" fill="#65776e" font-size="10">${e(edge.condition ?? edge.label ?? '')}${edge.active === false ? ' inactive' : ''}</text></g>`; }).join('');
    const nodes = g.nodes.map(n => { const h = n.fields ? 68 + n.fields.length * 20 : 75; const status = statuses[n.id]; return `<g data-node="${e(n.id)}" tabindex="0" role="button" aria-label="Select ${e(n.label)}" transform="translate(${n.x},${n.y})"><rect width="195" height="${h}" rx="9" fill="#fff" stroke="${selected === n.id ? '#237768' : status === 'failed' ? '#b95840' : '#d6ded9'}" stroke-width="${selected === n.id ? 2 : 1.2}"/><rect x="12" y="14" width="23" height="23" rx="5" fill="#eaf0ec"/><path d="M18 21h11m-11 5h11m-11 5h7" stroke="#527066" stroke-width="1.5"/><text x="43" y="29" font-size="11.5" font-weight="600" fill="#253d35">${e(n.label.length > 23 ? n.label.slice(0, 22) + '...' : n.label)}</text><text x="14" y="55" font-size="10" fill="#738079">${e(status ?? n.role)}</text>${(n.fields ?? []).map((f, i) => `<text x="14" y="${80 + i * 20}" font-size="11" fill="#62746b">${e(f)}</text>`).join('')}</g>`; }).join('');
    return `<svg viewBox="0 0 ${width} ${height}" class="graph-svg" role="img" aria-label="${model ? 'Semantic model' : 'Dependency graph'}"><defs><marker id="graph-arrow" markerWidth="7" markerHeight="7" refX="6" refY="3.5" orient="auto"><path d="M0 0L7 3.5L0 7Z" fill="#81948c"/></marker><pattern id="dots" width="18" height="18" patternUnits="userSpaceOnUse"><circle cx="2" cy="2" r=".6" fill="#dfe5e1"/></pattern></defs><rect width="100%" height="100%" fill="#fafcfb"/><rect width="100%" height="100%" fill="url(#dots)"/>${edges}${nodes}</svg>`;
}
