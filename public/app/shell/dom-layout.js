...
export function applyShellDOM(p, lab, ui) {
    ...
    ws.dataset.swapped = String(r.requested.swapped);
    ws.dataset.mobile = ui.mobile;
    ws.dataset.narrow = String(r.narrow);
    ws.dataset.output = r.outputSize;
    ws.dataset.anchor = r.outputAnchor;
    ws.classList.toggle('focus-lab', ui.focus);
    ...
    const anchor = document.querySelector('#' + r.outputAnchor + '-output'), output = document.querySelector('#output-dock');
    if (output.parentElement !== anchor)
        anchor.append(output);
    document.querySelectorAll('.output-anchor').forEach(el => el.hidden = el !== anchor || r.outputSize === 'closed');
    ...
    document.querySelector('#column-separator')?.setAttribute('aria-valuenow', String(r.split));
    document.querySelector('#drawer-separator')?.setAttribute('aria-valuenow', String(r.outputAnchor === 'right' ? r.requested.outputWidth : Math.round(height)));
    document.querySelector('#drawer-separator')?.setAttribute('aria-orientation', r.outputAnchor === 'right' ? 'vertical' : 'horizontal');
    document.querySelector('#tool-separator')?.setAttribute('aria-valuenow', String(r.toolWidth));
    document.querySelectorAll('[data-mobile]').forEach(b => b.classList.toggle('active', { question: 'context', lab: 'artifact', drawer: 'output', tools: 'tools' }[b.dataset.mobile] === ui.mobile));
}
