...
export function resultPanel(ctx) {
    if (ctx.busy)
...
export function explanationPanel(ctx) {
    const q = ctx.current;
    if (!ctx.revealed)
...
export function visualPanel(ctx) {
    let html = `<div class="visual-panel"><span class="eyebrow">MAKE THE CONCEPT VISIBLE</span>${steps(ctx.current.visual?.steps ?? ctx.current.concept)}${ctx.current.visual?.detail ? `<p>${e(ctx.current.visual.detail)}</p>` : ''}`;
    if (['dag-editor', 'architecture-editor', 'semantic-model'].includes(ctx.current.renderer))
        html += graphSVG(ctx.graph(), ctx.selectedNode, ctx.runStatuses);
    if (ctx.current.renderer === 'git-visual')
        html += gitGraphSVG(ctx.git(), ctx.selectedCommit);
    if (ctx.current.renderer === 'pyspark-editor')
        html += `<h3>Expected sample output</h3>${table(ctx.current.fixture?.expected ?? [])}${pre(ctx.current.fixture?.plan)}`;
    return html + '</div>';
}
export function deepnotePanel(ctx) {
