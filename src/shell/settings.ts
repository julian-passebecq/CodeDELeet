...
import type { DeepnoteLink,Pack,Question,Store,Workspace } from '../types.js';
import { download,icon,notice,options } from '../ui.js';
export interface Bridge {
 syncImportedDraft:()=>void;
  ...
  readJSONFile: (input: HTMLInputElement, apply: (value: any) => void) => Promise<void>;
  builtins: Pack[];
  recalc: () => void;
  ...
  renderDrawer: () => void;
  handleShellChange: (t: HTMLInputElement) => boolean;
  workspace: Workspace;
  applyLayout: () => void;
  runEpoch: number;
  busy: boolean;
...
export function settings(ctx: Bridge): void {
    ...
    if (ctx.storageBlocked) {
        ...
        ctx.$('#storage-reset').onclick = () => { if (confirm('Start fresh local storage? Download your raw backup first. This replaces the unreadable active value, not the pre-V2 snapshot.')) {
            ctx.storageBlocked = false;
            ctx.persist(true);
            dialog.close();
        } };
    }
    ctx.$('#backup-import').onchange = () => void ctx.readJSONFile(ctx.$('#backup-import'), value => { ctx.store = mergeStores(ctx.store, value, ctx.builtins); ctx.recalc(); ctx.persist(true); dialog.close(); ctx.syncImportedDraft(); ctx.toast('Backup merged. Newer Practice drafts and Learn records win; presentation settings stay on this device.'); });
    ...
    ctx.$('#map-import').onchange = () => void ctx.readJSONFile(ctx.$('#map-import'), value => { if (value.schemaVersion !== 1 || !value.links || typeof value.links !== 'object' || Array.isArray(value.links))
        throw Error('Expected schemaVersion 1 with a links object.'); const clean: Record<string, DeepnoteLink[]> = {}; for (const [id, links] of Object.entries(value.links)) {
    ...
    } ctx.store.settings.deepnoteMap = { ...ctx.store.settings.deepnoteMap, ...clean }; ctx.persist(true); dialog.close(); ctx.renderDrawer(); ctx.toast('Deepnote links saved. Blank URLs remain hidden.'); });
    dialog.onchange = ev => { ctx.handleShellChange(ev.target as HTMLInputElement); };
    ...
    dialog.querySelectorAll<HTMLElement>('[data-export-pack]').forEach(b => b.onclick = () => { const p = ctx.store.customPacks.find(p => p.id === b.dataset.exportPack); download(p!.id + '.json', JSON.stringify(p, null, 2)); });
}

export async function readJSONFile(ctx: Bridge, input: HTMLInputElement, apply: (value: any) => void): Promise<void> { const file = input.files?.[0]; if (!file)
    return; try {
