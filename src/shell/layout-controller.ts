...
export interface TransientShell {focus:boolean;navOverlay:boolean;focusSnapshot?:{tool:ToolId|null;expanded:boolean;navOverlay:boolean;outputSize:OutputSize;preferences?:Presentation};tool:ToolId|null;toolExpanded:boolean;mobile:'context'|'artifact'|'output'|'tools';}
export interface EffectiveLayout {preset:Preset;requested:ModePreferences;context:boolean;split:number;outputAnchor:OutputAnchor;outputSize:OutputSize;toolWidth:number;pinned:boolean;navWidth:number;navState:'expanded'|'compact'|'overlay'|'drawer'|'hidden';navOverlayWidth:number;narrow:boolean;focus:boolean;}
...
export function resolveLayout(p:Presentation,lab:Workspace,width:number,ui:TransientShell):EffectiveLayout{
 ...
 const nav=resolveNavigator(p,width,ui);const navWidth=nav.width;
...
export function newTransient():TransientShell{return {focus:false,navOverlay:false,tool:null,toolExpanded:false,mobile:'artifact'};}
export function toggleFocus(ui:TransientShell,p?:Presentation):void{
 if(!ui.focus){ui.focusSnapshot={tool:ui.tool,expanded:ui.toolExpanded,navOverlay:ui.navOverlay,outputSize:'closed',preferences:p?copy(p):undefined};ui.focus=true;ui.navOverlay=false;ui.tool=null;ui.toolExpanded=false;}
 else {if(p&&ui.focusSnapshot?.preferences){const before=ui.focusSnapshot.preferences;p.layouts=copy(before.layouts);p.modes=copy(before.modes);p.navWidth=before.navWidth;p.navCollapsed=before.navCollapsed;}ui.focus=false;ui.navOverlay=ui.focusSnapshot?.navOverlay??false;ui.tool=ui.focusSnapshot?.tool??null;ui.toolExpanded=ui.focusSnapshot?.expanded??false;ui.focusSnapshot=undefined;}
}
...
export function resolveNavigator(p:Presentation,width:number,ui:TransientShell):{state:EffectiveLayout['navState'];width:number;overlayWidth:number;expanded:boolean}{
 const overlayWidth=Math.min(320,Math.max(240,width-32));
 if(ui.focus)return {state:'hidden',width:0,overlayWidth:0,expanded:false};
 if(width<760)return {state:ui.navOverlay?'drawer':'hidden',width:0,overlayWidth:ui.navOverlay?overlayWidth:0,expanded:ui.navOverlay};
 if(width<1200)return {state:ui.navOverlay?'overlay':'compact',width:56,overlayWidth:ui.navOverlay?overlayWidth:0,expanded:ui.navOverlay};
 return {state:p.navCollapsed?'compact':'expanded',width:p.navCollapsed?56:p.navWidth,overlayWidth:0,expanded:!p.navCollapsed};
}
export function toggleNavigator(p:Presentation,ui:TransientShell,width:number):void {
 if(ui.focus)toggleFocus(ui,p);
 if(width>=1200)p.navCollapsed=!p.navCollapsed;
 else {ui.navOverlay=!ui.navOverlay;if(ui.navOverlay){ui.tool=null;ui.toolExpanded=false;ui.mobile='artifact';}}
}
