...
import type {Lesson,LessonBlock,LessonDiagram,LessonProgress} from './types.js';
import {blockTitle} from './validate.js';
const list=(items:string[]):string=>'<ul>'+items.map(x=>'<li>'+e(x)+'</li>').join('')+'</ul>';
function gridTable(columns:string[],rows:string[][]):string{return `<div class="lesson-table-scroll" tabindex="0" role="region" aria-label="Scrollable comparison table"><table><thead><tr>${columns.map(c=>'<th scope="col">'+e(c)+'</th>').join('')}</tr></thead><tbody>${rows.map(r=>'<tr>'+r.map(c=>'<td>'+e(c)+'</td>').join('')+'</tr>').join('')}</tbody></table></div>`;}
/** Two-column graph stays legible on phones. Text below exposes every edge. */
export function lessonDiagram(d:LessonDiagram,key:string):string {
...
export function renderBlock(b:LessonBlock,lessonId:string,questions:Question[]):string {
 let html='';
 switch(b.kind){
  case 'intro':html='<p class="lesson-intro">'+e(b.text)+'</p>';break;
  case 'concept':html='<p>'+e(b.text)+'</p>'+(b.mentalModel?'<div class="mental-model"><span class="eyebrow">MENTAL MODEL</span><p>'+e(b.mentalModel)+'</p></div>':'');break;
  case 'bullets':case 'keyTakeaways':html=list(b.items);break;
  case 'steps':html='<ol class="lesson-steps">'+b.steps.map(s=>'<li><strong>'+e(s.title)+'</strong><p>'+e(s.text)+'</p></li>').join('')+'</ol>';break;
  case 'code':html=`<div class="lesson-code-head"><span>${e(b.language)}</span><button class="text-button" data-copy-block="${e(b.id)}" aria-label="Copy ${e(b.title??'code example')}">Copy code</button></div><pre class="code-block" tabindex="0"><code>${e(b.code)}</code></pre>${b.caption?'<p class="block-caption">'+e(b.caption)+'</p>':''}`;break;
  case 'table':html=gridTable(b.columns,b.rows);break;
  case 'diagram':html=lessonDiagram(b.diagram,lessonId+'-'+b.id)+(b.caption?'<p class="block-caption">'+e(b.caption)+'</p>':'');break;
  case 'comparison':html=`<div class="lesson-comparison">${[b.left,b.right].map(col=>'<div><h3>'+e(col.title)+'</h3>'+list(col.items)+'</div>').join('')}</div>`;break;
  case 'whenToUse':html=gridTable(['When you see this','Consider this approach'],b.items.map(i=>[i.signal,i.choice]));break;
  case 'pitfall':html='<p>'+e(b.text)+'</p>'+(b.correction?'<p><strong>Instead: </strong>'+e(b.correction)+'</p>':'');break;
  case 'workedExample':html='<ol>'+b.steps.map(s=>'<li>'+e(s)+'</li>').join('')+'</ol>'+(b.result?'<p class="example-result"><strong>Result: </strong>'+e(b.result)+'</p>':'');break;
  case 'checkpoint':html='<p>'+e(b.question)+'</p>'+(b.choices?list(b.choices):'')+`<details class="lesson-checkpoint"><summary>Reveal reasoning</summary><p><strong>${e(b.answer)}</strong></p><p>${e(b.explanation)}</p></details><p class="block-caption">Self-check only. This does not score a Practice attempt.</p>`;break;
  case 'relatedPractice':html=b.exerciseIds.map(id=>{const q=questions.find(q=>q.id===id);return q?`<button class="related-exercise" data-question="${e(id)}"><span><strong>${e(q.title)}</strong><small>${e(q.technology??q.topic)} / ${e(q.difficulty)}</small></span><span aria-hidden="true">&rarr;</span></button>`:'';}).join('');break;
 }
...
export function lessonHTML(lesson:Lesson,progress:LessonProgress|undefined,questions:Question[],siblings:Lesson[]):string {
