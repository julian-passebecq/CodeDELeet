/** V2.4 taxonomy. Stable exercise IDs remain owned by the original packs. */
import type { Question, Workspace } from '../types.js';
export interface CategoryDefinition { id: string; workspace: Workspace; title: string; description: string; icon: string; order: number; }
const families: Record<Workspace, [string,string,string,string][]> = {
 code: [
...
export const CATEGORIES: CategoryDefinition[] = Object.entries(families).flatMap(([workspace, rows]) => rows.map(([id,title,description,icon], order) => ({id,title,description,icon,order,workspace:workspace as Workspace})));
export const categoriesFor = (workspace:Workspace):CategoryDefinition[] => CATEGORIES.filter(c=>c.workspace===workspace);
export const categoryFor = (workspace:Workspace,id?:string):CategoryDefinition|undefined => CATEGORIES.find(c=>c.workspace===workspace&&c.id===id);
...
export const EXERCISE_CATEGORIES:Record<string,{categoryId:string;subcategory:string}> = Object.fromEntries(groups.flatMap(([categoryId,subcategory,ids])=>ids.map(id=>[id,{categoryId,subcategory}])));
export function association(q:Question):{categoryId:string;subcategory:string;imported:boolean} {
 const mapped=EXERCISE_CATEGORIES[q.id];
 if(mapped&&categoryFor(q.workspace,mapped.categoryId))return {...mapped,imported:false};
 if(typeof q.categoryId==='string'&&categoryFor(q.workspace,q.categoryId))return {categoryId:q.categoryId,subcategory:typeof q.subcategoryId==='string'?q.subcategoryId.slice(0,80):'Imported / Other',imported:true};
 return {categoryId:categoriesFor(q.workspace)[4].id,subcategory:'Imported / Other',imported:true};
}
export function validateTaxonomy(questions:Question[]):void {
 for(const lab of Object.keys(families) as Workspace[])if(categoriesFor(lab).length!==5)throw Error('Exactly five primary categories required: '+lab);
 if(new Set(CATEGORIES.map(c=>c.id)).size!==20)throw Error('Duplicate category ID');
 for(const [id,map]of Object.entries(EXERCISE_CATEGORIES)){const q=questions.find(q=>q.id===id);if(!q||!categoryFor(q.workspace,map.categoryId))throw Error('Invalid exercise/category association: '+id);}
}
