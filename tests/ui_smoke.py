...
def go(page,qid):
    ...
    # V2.2 intentionally restores the previous specialist tab; these legacy scenarios start in Workspace.
    click_tab(page,'lab','Workspace')
def close_tools(page):
    if page.locator('#tool-panel').is_visible():page.locator('#tool-header [data-shell="tool-close"]').click()
def click_tab(page,group,tab):
    if group=='drawer':
        if tab=='Results':
            if not page.locator('#output-dock').is_visible():page.locator('#tool-rail [data-shell="output-toggle"]').click()
            return
        if page.locator('#tool-panel').is_visible() and page.locator('#tool-panel-title').inner_text()==tab:return
        page.locator(f'#tool-rail [data-tool="{tab}"]').click()
        return
    close_tools(page)
    if group=='left' and not page.locator('#question-body').is_visible():page.locator('[data-shell="context-toggle"]').first.click()
    page.locator(f'[data-{group}-tab="{tab}"]').click()
...
with sync_playwright() as p:
    ...
    def reveal_safety():
        ...
        assert BYID['sql-paid-revenue']['solution'] not in page.locator('#tool-body').inner_text()
        page.locator('[data-action="reveal"]').click();assert BYID['sql-paid-revenue']['explanation'] in page.locator('#tool-body').inner_text()
    ...
    def layout():
        close_tools(page);before=int(page.locator('#column-separator').get_attribute('aria-valuenow'));page.locator('#column-separator').focus();page.keyboard.press('ArrowRight');page.wait_for_timeout(260);assert saved(page)['settings']['workstation']['layouts']['code.solve']['split']==before+2
        click_tab(page,'drawer','Results');page.locator('[data-shell="output-close"]').click();assert not page.locator('#output-dock').is_visible()
        click_tab(page,'drawer','Results');assert page.locator('#output-dock').is_visible()
    ...
    for q in (QS if os.getenv('SKIP_MATRIX')!='1' else []):
        def matrix(q=q):
            ...
            for tab in ['Visual','Explanation','Results']:click_tab(page,'drawer',tab)
            close_tools(page)
    ...
    def deepnote():
        go(page,'spark-dedup');assert page.locator('#tool-rail [data-tool="Deepnote"]').count()==0;assert page.locator('a[href^="https://deepnote.com"]').count()==0
        page.locator('[data-action="settings"]').click();payload={'schemaVersion':1,'links':{'spark-dedup':[{'type':'exercise','label':'My configured notebook','url':'https://deepnote.com/workspace/my-team/project/my-project'}]}}
        page.locator('#map-import').set_input_files({'name':'mapping.json','mimeType':'application/json','buffer':json.dumps(payload).encode()});page.wait_for_timeout(100);click_tab(page,'drawer','Deepnote');assert page.locator('a[href^="https://deepnote.com/workspace/my-team"]').count()==1
...
(ROOT/'evidence/v22/ui-legacy-report.json').write_text(json.dumps(report,indent=2))
