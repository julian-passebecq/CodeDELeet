...
def choose_theme(p,value):
 tool(p,'Theme');p.locator('input[name="app-theme"][value="'+value+'"]').check();close_tool(p)
def go(p,id,mode='work'):
 ...
 p.wait_for_function('(t)=>document.querySelector("h1")?.textContent===t',arg=BYID[id]['title']);p.locator('button[data-mode="'+mode+'"]').click()
...
with sync_playwright() as pw:
 ...
 def undo():
  ...
  p.locator('button[data-mode="inspect"]').click();p.locator('button[data-mode="case"]').click();tool(p,'Notes');p.locator('#notes').fill('Persistent note');close_tool(p);choose_theme(p,'fluent-soft');p.locator('[data-action="focus"]').click();p.locator('[data-action="focus"]').click();p.locator('button[data-mode="work"]').click()
 ...
 def dock_modes():
  ...
  p.locator('#output-size').select_option('compact');p.locator('#output-anchor').select_option('artifact');p.locator('#drawer-separator').focus();p.keyboard.press('ArrowUp');before=state(p)['settings']['workstation']['layouts']['model.measures'];p.locator('button[data-mode="work"]').click();p.locator('button[data-mode="inspect"]').click();assert state(p)['settings']['workstation']['layouts']['model.measures']==before
 ...
 def theme():
  ...
  for theme in ['sage-light','fluent-light','fluent-soft','slate-dark']:
   choose_theme(p,theme);assert p.locator('html').get_attribute('data-theme')==theme;colors[theme]=p.locator('.terminal-console').evaluate('(el)=>getComputedStyle(el).backgroundColor');overflow(p)
  assert len(set(colors.values()))==4;assert state(p)['drafts']['v2-powershell-csv']['labState']==before
  choose_theme(p,'fluent-soft');p.locator('[data-action="settings"]').click();p.locator('#terminal-theme').select_option('dark');p.locator('#density').select_option('compact');p.locator('#text-size').select_option('15');p.locator('[aria-label="Close settings"]').click();assert p.locator('html').get_attribute('data-terminal')=='dark';assert state(p)['settings']['workstation']['terminal']=='dark';p.locator('[data-action="settings"]').click();p.locator('#terminal-theme').select_option('inherit');p.locator('[aria-label="Close settings"]').click();snap(p,'06-fluent-soft')
 check('Four themes apply without domain reset; terminal inheritance and override/settings persist',theme)
 def safe_links():
  go(p,'spark-dedup');assert p.locator('#tool-rail [data-tool="Deepnote"]').count()==0;p.locator('[data-action="settings"]').click();payload={'schemaVersion':1,'links':{'spark-dedup':[{'type':'exercise','label':'Configured companion','url':'https://deepnote.com/project/explicit-user-choice'}]}};p.locator('#map-import').set_input_files({'name':'safe.json','mimeType':'application/json','buffer':json.dumps(payload).encode()});tool(p,'Deepnote');link=p.locator('#tool-body a[href^="https://deepnote.com/project/"]');assert link.count()==1;assert 'noopener' in link.get_attribute('rel');close_tool(p);p.locator('[data-action="settings"]').click();payload['links']['spark-dedup'][0]['url']='https://deepnote.com.evil.invalid/app/fake';p.locator('#map-import').set_input_files({'name':'unsafe.json','mimeType':'application/json','buffer':json.dumps(payload).encode()});p.wait_for_function("document.querySelector('#toast').textContent.includes('Unsafe')");assert p.locator('#modal').is_visible();assert 'Unsafe' in p.locator('#toast').inner_text();p.locator('[aria-label="Close settings"]').click();tool(p,'Deepnote');assert link.count()==1;close_tool(p)
